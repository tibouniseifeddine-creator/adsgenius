...
- **Test suite fix (P14):** raised Vitest's `testTimeout` from 15s to 30s after real runs against a disposable Neon branch showed the fuller two-factor login-flow tests (7-9 real sequential HTTP requests plus bcrypt hashing per test) legitimately taking close to 15s against a small remote database -- not a hang or a logic bug, just not enough of a time budget for that much real, sequential network + crypto work. See `docs/TESTING.md`.
