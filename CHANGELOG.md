...
- **Test suite fix (P13):** diagnosed and fixed an intermittent `P1001`/`P2024` Prisma connection-pool exhaustion issue seen when running the integration tests against a real disposable Neon branch -- Vitest's default file-level parallelism let both integration test files open a separate database connection pool at the same time, which could overwhelm a small test branch's compute. `vitest.config.ts` now sets `fileParallelism: false` so integration test files run one at a time; see `docs/TESTING.md` for the full explanation and further troubleshooting steps.
- **Test suite fix (P14):** raised Vitest's `testTimeout` from 15s to 30s (later 45s, see P15) after real runs against a disposable Neon branch showed the fuller two-factor login-flow tests (7-9 real sequential HTTP requests plus bcrypt hashing per test) legitimately taking close to 15s against a small remote database -- not a hang or a logic bug, just not enough of a time budget for that much real, sequential network + crypto work. See `docs/TESTING.md`.
- **Registration robustness fix (P15):** real integration test runs against a disposable Neon branch surfaced a genuine bug, not just a test-timing issue -- `POST /api/auth/register`'s interactive transaction used Prisma's default 5-second budget, and a serverless Postgres compute waking from suspend can take longer than that to answer its first query, which aborted an otherwise-valid registration with a `P2028` "Transaction already closed" error. Raised to `{ maxWait: 10000, timeout: 20000 }` in both `api/index.ts` and `backend/src/server.ts`, so a slow cold start is survived instead of surfacing as a false registration failure -- this protects real users on the production database after any period of inactivity, not just this test suite. Vitest's `testTimeout` raised to 45s to match. See `docs/TESTING.md`.
