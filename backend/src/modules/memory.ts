...
export async function createMemory(auth: AuthContext, workspaceId: string, input: Record<string, unknown>, requestId: string) {
  ...
  const memoryType = text(input.memoryType, 'memoryType', 40) as MemoryType;
  if (!MEMORY_TYPES.has(memoryType)) throw new AppError('VALIDATION_ERROR', 'Unsupported memoryType.', 400);
  const scopeKey = text(input.scopeKey, 'scopeKey', 200);
  const sourceType = text(input.sourceType, 'sourceType', 80);
  const content = jsonObject(input.content, 'content');
  const approved = input.approved === true;
  const sourceRef = optionalText(input.sourceRef, 200);
  ...
  await prisma.auditLog.create({ data: { workspaceId, userId: auth.userId, action: 'ai.memory.create', entityType: 'AIMemory', entityId: id, afterJson: { memoryType, scopeKey, approved, sourceType }, requestReference: requestId } });
...
export async function listMemories(auth: AuthContext, workspaceId: string, options: { memoryType?: string; scopeKey?: string; approvedOnly?: boolean } = {}) {
  await requireWorkspaceAccess(auth, workspaceId);
  const memoryType = options.memoryType && MEMORY_TYPES.has(options.memoryType as MemoryType) ? options.memoryType : null;
...
export async function updateMemoryGovernance(auth: AuthContext, workspaceId: string, memoryId: string, input: Record<string, unknown>, requestId: string) {
  await requireWorkspaceAccess(auth, workspaceId);
  if (input.approved !== undefined && typeof input.approved !== 'boolean') throw new AppError('VALIDATION_ERROR', 'approved must be boolean.', 400);
  const score = input.confidence === undefined ? null : confidence(input.confidence);
  ...
  if (!rows[0]) throw new AppError('NOT_FOUND', 'Memory not found.', 404);
  await prisma.auditLog.create({ data: { workspaceId, userId: auth.userId, action: 'ai.memory.governance.update', entityType: 'AIMemory', entityId: memoryId, afterJson: { approved: input.approved, confidence: score }, requestReference: requestId } });
  return normalizeMemory(rows[0]);
}

export async function deleteMemory(auth: AuthContext, workspaceId: string, memoryId: string, requestId: string) {
  await requireWorkspaceAccess(auth, workspaceId);
  const result = await prisma.$executeRaw(Prisma.sql`UPDATE "ai_memories" SET "deleted_at"=CURRENT_TIMESTAMP,"updated_at"=CURRENT_TIMESTAMP WHERE "id"=${memoryId}::uuid AND "workspace_id"=${workspaceId} AND "deleted_at" IS NULL`);
  if (result !== 1) throw new AppError('NOT_FOUND', 'Memory not found.', 404);
  await prisma.auditLog.create({ data: { workspaceId, userId: auth.userId, action: 'ai.memory.delete', entityType: 'AIMemory', entityId: memoryId, requestReference: requestId } });
}

export async function recordLearning(auth: AuthContext, workspaceId: string, input: Record<string, unknown>, requestId: string) {
  await requireWorkspaceAccess(auth, workspaceId);
  const subjectType = text(input.subjectType, 'subjectType', 80);
  const outcome = text(input.outcome, 'outcome', 30) as LearningOutcome;
  if (!OUTCOMES.has(outcome)) throw new AppError('VALIDATION_ERROR', 'Unsupported learning outcome.', 400);
  const summary = text(input.summary, 'summary', 2000);
  const evidence = input.evidence === undefined ? {} : jsonObject(input.evidence, 'evidence');
  const subjectId = optionalText(input.subjectId, 100);
  const score = confidence(input.confidence);
  const id = randomUUID();
  ...
  await prisma.auditLog.create({ data: { workspaceId, userId: auth.userId, action: 'ai.learning.record', entityType: 'AILearningRecord', entityId: id, afterJson: { subjectType, subjectId, outcome }, requestReference: requestId } });
...
export async function listLearning(auth: AuthContext, workspaceId: string, subjectType?: string, subjectId?: string) {
  await requireWorkspaceAccess(auth, workspaceId);
  const type = subjectType?.trim() || null;
  const id = subjectId?.trim() || null;
  const rows = await prisma.$queryRaw<any[]>(Prisma.sql`SELECT * FROM "ai_learning_records" WHERE "workspace_id"=${workspaceId} AND (${type}::text IS NULL OR "subject_type"=${type}) AND (${id}::text IS NULL OR "subject_id"=${id}) ORDER BY "created_at" DESC LIMIT 100`);
  return rows.map(normalizeLearning);
}
