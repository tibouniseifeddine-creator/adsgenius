...
function refreshTokenFrom(request: IncomingMessage): string | undefined {
...
export async function authenticate(request: IncomingMessage): Promise<AuthContext> {
  ...
  const session = await prisma.authSession.findUnique({ where: { accessTokenHash: hashToken(token), user: true } });
  if (!session || session.revokedAt || session.expiresAt <= new Date()) {
    throw new AppError('UNAUTHORIZED', 'The access session is invalid or expired.', 401);
  }
...
export async function refresh(request: IncomingMessage, response: ServerResponse, requestId: string) {
  ...
  if (!current || current.revokedAt || current.refreshExpiresAt <= new Date()) {
    throw new AppError('UNAUTHORIZED', 'The refresh session is invalid or expired.', 401);
  }
...
export async function me(request: IncomingMessage) {
  ...
  return {
    ...
    workspaces: memberships.map(({ workspace, role }) => ({ ...workspace, role })),
  };
}
