...
`two-factor-login-flow.test.ts`'s fuller tests (a complete setup-then-login flow, or a recovery-code login) each make 7-9 real sequential HTTP requests through the Express app, and two of them do a bcrypt password hash/compare on top of that. Against a real remote database -- especially a small disposable test branch -- that is genuinely tens of seconds of real work, not a hang. `vitest.config.ts` sets `testTimeout: 45000` (45s) for exactly this reason -- room for that plus a cold-start registration (see the next section). If you still see a timeout after that (a slower machine, a more distant database region, or a smaller compute than the one this suite was tuned against), raise it further rather than treating it as a bug in the 2FA code itself -- the equivalent unit tests in `two-factor.test.ts` (no database involved) pass in milliseconds, which is the real check that the TOTP logic itself is correct.

### Troubleshooting: registration fails with `P2028` ("Transaction already closed")

If `POST /api/auth/register` (and so `registerUser()` in the integration tests) fails with a Prisma `P2028` error mentioning a 5000ms interactive-transaction timeout, this was a real bug that has since been fixed, not something you need to work around: Prisma's default budget for an interactive `$transaction(...)` is only 5 seconds, and a serverless Postgres compute waking up from being fully suspended (Neon, in particular, on a small/free-tier branch) can genuinely take longer than that for the very first query to land. The registration transaction in `api/index.ts` (and its mirror in `backend/src/server.ts`) now passes `{ maxWait: 10000, timeout: 20000 }`, giving a cold compute real room to wake up before the transaction is judged to have failed. If you still see `P2028` after pulling that fix, the database is taking longer than 20s to respond to a simple insert, which points to a genuinely overloaded or misconfigured database rather than an ordinary cold start.
